...
def main():
    ...
    symbol_quantities = parse_formula(formula, periodic_table)
    
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    compute_the_mass = compute_molar_mass(symbol_quantities, periodic_table)

    # Compute the number of moles in the sample.
    molar_mass = grams_mass / compute_the_mass
    # Print the molar mass.
    print(f'{compute_the_mass:.5f} grams/mole')
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for list in symbol_quantity_list:
        atom = list[NAME_INDEX]
        number_of_atoms = list[ATOMIC_MASS_INDEX]
        element_attributes = periodic_table_dict[atom]
        atomic_mass = element_attributes[1]
        t_atomic_mass = atomic_mass * number_of_atoms
        total_mass = total_mass + t_atomic_mass
